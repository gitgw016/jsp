...
public class CommonFileUploadServlet extends HttpServlet {
	...
	private final String UPLOAD_DIRECTORY = "upload"+File.separator+"comm";
	// 업로드에 사용될(임시데이터) 메모리 크기 제한
	private final int MEMORY_THRESHOLD = 1024*1024*3; // 3mb
	// 업로드되는 한개의 파일 크기 제한
	private final int MAX_FILE_SIZE = 1024*1024*20;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!ServletFileUpload.isMultipartContent(request)) {
			...
			pw.write("오류 : multipart-formdata 타입이 아니다.");
			pw.flush();
			return;
		}
		...
		// 임시 저장소 위치 지정. 임시 저장소의 파일은 실제 파일이 업로드 완료되고 나면 삭제됨.
			...
			if(formItems != null && formItems.size() > 0) {
				for(FileItem item : formItems) {
					if(!item.isFormField()) {
						...
						// 매개변수로 넘겨받은 경로와 파일이름으로 바이너리에 넘겨받은 파일 업로드
						item.write(storeFile);
						request.setAttribute("request", "File upload Succed");
