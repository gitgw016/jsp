...
<form action="main.jsp?page=memberSubmit" method="post">
	<table>
		...
		<tr>
			<td>나이</td>
			<td><input type="number" name="u_age" value="${updateMember.u_age}" required/></td>
		</tr>
		<tr>
			<td>성별</td>
			<td>
				<label>
					<input <c:out value="${updateMember.u_gender eq 'male' ? 'checked' : ''}"/> type="radio" name="u_gender" value="male"/> 남성
				</label>
					<input <c:out value="${updateMember.u_gender eq 'female' ? 'checked' : ''}"/> type="radio" id="female" name="u_gender" value="female"/>
