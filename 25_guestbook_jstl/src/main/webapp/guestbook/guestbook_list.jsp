...
<s:query var="r" dataSource="jdbc/MySqlDB" sql="SELECT count(*) AS count FROM test_guest"/>
...
<jsp:useBean id="bookList" class="java.util.ArrayList" type="List<GuestBook>"/>
<s:query var="rs" dataSource="jdbc/MySqlDB" startRow="${pm.cri.startRow}" maxRows="${pm.cri.perPageNum}">SELECT * FROM test_guest ORDER BY id DESC</s:query>
...
<c:choose>
	...
	<c:otherwise>등록된 메세지가 존재하지 않습니다.</c:otherwise>
</c:choose>
<!-- 방명록 리스트 정보 출력 end -->
