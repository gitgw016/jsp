...
<body>
	...
	${param.phone} <br/><hr/>
	<c:catch var="e">
		<s:setDataSource var="conn" driver="com.mysql.cj.jdbc.Driver" url="jdbc:mysql://localhost:3306/smartWeb" user="smartWeb" password="12345"/>
		<h3>${conn}</h3>
		<!-- executeQuery -->
		<%-- <s:query var="r" dataSource="${conn}">SELECT * FROM test_member</s:query> --%>
		<h3>전체 리스트</h3>
		...
		INSERT INTO test_member VALUES(null,'${param.id}','${param.pass}','${param.name}','${param.addr}','${param.phone}','${param.gender}',${param.age},now())
		</s:update>
		<h3>삽입 후 실행 결과 검색</h3>
		...
		<c:forEach var="d" items="${rs.rows}">
			${d.num} &nbsp; ${d.id} &nbsp; ${d.pass} &nbsp; ${d.name} &nbsp; ${d.addr} &nbsp; ${d.phone} &nbsp; ${d.gender} &nbsp; ${d.age} &nbsp; <f:formatDate value="${d.regdate}" pattern="yyyy-MM-dd hh:mm:ss"/><br/>
		</c:forEach>
			<c:set var="id" value="qoalsrn"/>
			<c:set var="pass" value="123"/>
		<s:query var="rss" dataSource="jdbc/MySqlDB"> SELECT * FROM test_member WHERE id = ? AND pass = ?
			<s:param>${id}</s:param>
		...
		<s:update var="result" dataSource="jdbc/MySqlDB" sql="DELETE FROM test_member WHERE id='${param.id}'"/><br/>
		result : ${result}
	</c:catch>
	<c:if test="${!empty e}">에러 메시지 : <c:out value="${e.message}"/></c:if>
</body>
</html>