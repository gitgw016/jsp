<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="java.sql.*,java.util.*,util.*,vo.*"%>
<%
	int currentPage = 1;
	int pageCount = 5;
	int totalCount = 0;
...
<%
	...
	String sql = "SELECT * FROM test_guest ORDER BY id DESC limit ?,?";
	try{
		...
		while(rs.next()){
			GuestBook vo = new GuestBook(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4));
			bookList.add(vo);
		}
	}catch(Exception e){}
%>
...
<hr>
<%	try{
	sql = "SELECT count(*) FROM test_guest;";
	pstmt = conn.prepareStatement(sql);
	rs = pstmt.executeQuery();
	if(rs.next()){
